...
import { IoPlay, IoPause, IoChevronDownSharp } from 'react-icons/io5';
...
function MusicPlayer() {
  ...
  return (
    <div className={styles.wrapper}>
      <button
        className={`${styles.toggle} ${isCollapsed ? styles.buttonCollapsed : ''}`}
        onClick={toggleCollapsed}
        aria-label={isCollapsed ? 'Expand player' : 'Collapse player'}
      >
        <IoChevronDownSharp className={isCollapsed ? styles.rotate : ''} />
      </button>
      <footer
        className={`${styles.player} ${isCollapsed ? styles.collapsed : ''}`}
